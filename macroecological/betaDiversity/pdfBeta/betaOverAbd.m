function betaValueAbd = betaOverAbd(dataCellCell,abdRange)

% Created by Li Jie
% Date: 2018-5-2
% Calculate: Beta over Abundance for all people and combine them as a vector.
...
for i = 1:size(dataCellCell,2)
    ...
    for iii = 2:length(abdRange)-1
        ...
        if length(dataIndexBin) == 0
            ...
            speciesIndexBin_pre = speciesIndexBin_this; clear speciesIndexBin_this  % BUG: CLEAR is VERY important!!!
        end   
    end
end

betaValueAbd = betaDiversityPeople_Bins(1,:);
for ii = 2:size(dataCellCell,2)
    betaValueAbd = [betaValueAbd,betaDiversityPeople_Bins(ii,:)];
end
