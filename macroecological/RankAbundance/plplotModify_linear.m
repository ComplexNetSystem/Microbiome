function h=plplotModify_linear(x, xmin, alpha, index)
...
%% Plot in linear
        figure;
        h(1) = plot(c(:,1),c(:,2),'LineWidth',2);
%        h(1) = plot(c(:,1),c(:,2),'bo','MarkerSize',8,'MarkerFaceColor',[1 1 1]); hold on;
%        h(2) = plot(cf(:,1),cf(:,2),'k--','LineWidth',2); hold off;
        ylabel('epdf','FontSize',16);
        xlabel('Value','FontSize',16);
        title(['OTU',num2str(index)],'FontSize',16);
